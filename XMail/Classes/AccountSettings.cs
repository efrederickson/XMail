...
namespace XMail
{
    ...
    public class AccountSettings : IList<AccountSettings.AccountInfo>
    {
        ...
        /// <summary>
        /// Saves the Email account settings to a file.
        /// </summary>
        ...
        public static void Save(string file, AccountSettings c)
        {
            try
            {
                ...
                foreach (AccountInfo ai in c.Accounts)
                {
                    ai.Password = IExtendFramework.Encryption.ASCIIProvider.Encrypt(ai.Password);
                }
                xs.Serialize(writer, c);
                //Serializer.Serialize(c, file);
                writer.Flush();
                writer.Close();
            }
            catch (Exception ex)
                ...
                TaskDialog.Show(new TaskDialogOptions()
                                {
                                    Title = "Error!",
                                    Content = ex.Message,
                                    ExpandedInfo = "Full Error Details: " + ex.ToString()
                                });
            }
        }
        ...
        public static AccountSettings Load(string file)
        {
            ...
            try
            {
                //c = (AccountSettings)Serializer.Deserialize(file);
                c = (AccountSettings)xs.Deserialize(reader);
                foreach (AccountInfo ai in c.Accounts)
                {
                    ai.Password = IExtendFramework.Encryption.ASCIIProvider.Decrypt(ai.Password);
                }
            }
            catch (Exception)
            {
