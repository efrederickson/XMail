...
namespace XMail
{
    ...
    public class Pop3Controller
    {
        ...
        public void Connect(AccountSettings.AccountInfo accountInfo)
        {
            if (accountInfo != null && accountInfo.MailAccountType == AccountType.POP3)
            {
                ...
                if (ssl)
                {
                    if (useInPort)
                    {
                        this.Pop3Client.ConnectSsl(serverName, port, user, password);
                    }
                    else
                    {
                        this.Pop3Client.ConnectSsl(serverName, user, password);
                    }
                }
                else
                {
                    if (useInPort)
                    {
                        this.Pop3Client.Connect(serverName, port, user, password);
                    }
                    else
                    {
                        this.Pop3Client.Connect(serverName, user, password);
                    }
                }
            }
        }
        ...
        public BindingList<MailMessage> RetrieveMessages(int beginIndex, int increment)
        {
            ...
            int messageCount = this.Pop3Client.MessageCount;
            for (int i = beginIndex; i <= beginIndex + increment; i++)
            {
                if (i <= messageCount)
                {
                    header = this.Pop3Client.RetrieveHeaderObject(i);
        ...
        public void DeleteMessage(string messageId)
        {
            ...
            {
                Header header = this.Pop3Client.RetrieveHeaderObject(i);
            ...
            if (index > 0)
            {
                this.Pop3Client.DeleteMessage(index);
            }
        }
        ...
        public ActiveUp.Net.Mail.Message GetMessage(MailMessage mailMessage)
        {
            ...
            while (downIndex > 0 || upIndex <= messageCount)
            {
                ...
                {
                    message = this.Pop3Client.RetrieveMessageObject(downIndex);
                ...
                {
                    message = this.Pop3Client.RetrieveMessageObject(upIndex);
                    if (message.MessageId != null && message.MessageId.Equals(mailMessage.Id))
                    {
                        ret = message;
                        break;
        ...
        {
            return this.Pop3Client.MessageCount;
        }
